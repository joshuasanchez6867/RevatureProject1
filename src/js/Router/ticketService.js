...
//add router folder remove req and res
//path param should use ticket id not body req
...
const arbitrate = (req, res) => {
    if(!req.body.ticket_id || typeof req.body.ticket_id  !== typeof 'string'){//add for other memebrs too
...
const viewTickets = (req, res) => {
    ...
    .then((payload) => {
        if(payload.admin === 'admin') {//get all pending
            ...
            .then((data) => {
                res.status(200).send({PendingTickets: data.Item});
            })
            .catch(() => {
            ...
            .then((data) => {
                res.status(200).send({PreviousTickets: data.Item});
            })
            .catch(() => {
                res.status(401).send({error: 'Bad Request'});
            })
